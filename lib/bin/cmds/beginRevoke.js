...
exports.module = {
    ...
    handler: util_1.asyncHandler(function (argv) { return __awaiter(_this, void 0, void 0, function () {
        var _a, sdk, blockNumber, passphrase, tx, signed, _b, _c, _d, _e, newBlockNumber, summary, revocations, block, lastEndTime;
        return __generator(this, function (_f) {
            switch (_f.label) {
                ...
                case 3:
                    passphrase = _f.sent();
                    tx = src_1.createRequestRevokeTransaction(sdk, argv.delegatee, argv.quantity);
                ...
                case 7:
                    summary = _f.sent();
                    revocations = summary.revocations(argv.delegator, argv.delegatee)
                        .values;
                    return [4 /*yield*/, sdk.rpc.chain.getBlock(blockNumber)];
                case 8:
                    block = (_f.sent());
                    lastEndTime = revocations[revocations.length - 1].endTime;
                    console.log("Delegation will be revoked at", util_1.formatTimestamp(lastEndTime, {
...
function printSummary(sdk, blockNumber, delegator, delegatee, changes) {
    return __awaiter(this, void 0, void 0, function () {
        var _a, _b, ccsChanges, _c, cccChanges, summary, _d, balance, undelegated, delegationsTo, revocationsTo, cccBalance, _e, balance, undelegated, delegationsFrom, revocationsFrom;
        return __generator(this, function (_f) {
            switch (_f.label) {
                case 0:
                ...
                case 1:
                    ...
                    {
                        _d = summary.get(delegator), balance = _d.balance, undelegated = _d.undelegated, delegationsTo = _d.delegationsTo, revocationsTo = _d.revocationsTo;
                        ...
                        console.log.apply(console, __spread(["Pending Revocations (out):"], util_1.plusChangeArgs(revocationsTo.sum, ccsChanges)));
                    }
                    console.groupEnd();
                ...
                case 2:
                    cccBalance = _f.sent();
                    _e = summary.get(delegatee), balance = _e.balance, undelegated = _e.undelegated, delegationsFrom = _e.delegationsFrom, revocationsFrom = _e.revocationsFrom;
                    ...
                    console.log.apply(console, __spread(["Pending Revocations (in):"], util_1.plusChangeArgs(revocationsFrom.sum, ccsChanges)));
                    console.groupEnd();
                    console.log("Delegations between:", summary.delegations(delegator, delegatee).toLocaleString());
                    console.log.apply(console, __spread(["Pending revocations between:"], util_1.plusChangeArgs(summary.revocations(delegator, delegatee).sum, ccsChanges)));
